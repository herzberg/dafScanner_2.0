...
for i=startScanDaf:startScanDaf+numDafToScan-1
    ...
    perekBreakHeight = 50;
    if yoyoyo(1) ~= -1 && yo(2)-botline(2) < perekBreakHeight
        botline = yoyo;
        yo = yoyoyo;
    ...
    try
        start = topline(3);
        [corn,dim,start,dir] = nextJunctionFinder(im,topright,start,2,'d',imcloseCoef);
        ...
        if corn(2) > botline(2) % already reached bottom line, no need to check for right protrusion
            ...
            [corn2,dim2,start2,dir] = nextJunctionFinder(im,corn,start,dim,'r',imcloseCoef);
            cornPoint2 = [corn2(2) + (corn2(3)-corn2(2))/2, corn(2) + (corn(3)-corn(2))/2];
            gemPointsR = [gemPointsR; cornPoint2];
            %plot([gemPointsR(end-1,1),gemPointsR(end,1)],[gemPointsR(end-1,2),gemPointsR(end,2)]);

            %go down until botline
            gemPointsR = [gemPointsR; cornPoint2(1), botline(2) + 0.05*botline(1)];
            %plot([gemPointsR(end-1,1),gemPointsR(end,1)],[gemPointsR(end-1,2),gemPointsR(end,2)]);
        end
    catch e
    ...
    try
        start = topline(3);
        [corn,dim,start,dir] = nextJunctionFinder(im,topleft,start,2,'d',imcloseCoef);
        ...
        if corn(2) >= botline(2)-botlinebuffer % already reached bottom line, no need to check for left protrusion
            ...
            gemPointsL = [gemPointsL; topLeftPoint; cornPoint];
            %go left once
            plot([gemPointsL(end-1,1),gemPointsL(end,1)],[gemPointsL(end-1,2),gemPointsL(end,2)]);
            [corn2,dim2,start2,dir] = nextJunctionFinder(im,corn,start,dim,'l',imcloseCoef);
            
            cornPrime = [corn(1) corn(2)-20 corn(3)-20];
            [corn3,dim3,start3,dir3] = nextJunctionFinder(im,cornPrime,start,dim,'l',imcloseCoef);
            cornPoint2 = [corn2(2) + (corn2(3)-corn2(2))/2, corn(2) + (corn(3)-corn(2))/2];
            cornPoint3 = [corn3(2) + (corn3(3)-corn3(2))/2, corn(2) + (corn(3)-corn(2))/2];
            gemPointsL = [gemPointsL; cornPoint2; cornPoint3];
            ...
            %go down until botline
            gemPointsL = [gemPointsL; cornPoint2(1), botline(2) + 0.05*botline(1)];
            %plot([gemPointsL(end-1,1),gemPointsL(end,1)],[gemPointsL(end-1,2),gemPointsL(end,2)]);
        end
    catch e
    ...
    try 
        ...
        str = ['./results/cutoutGemara/' mesechtaName '/' mesechtaName '_' int2str(i) '.png'];
        ...
        str = ['./results/cutoutRashi/' mesechtaName '/' mesechtaName '_' int2str(i) '.png'];
        ...
        if hasTos
            ...
            str = ['./results/cutoutTosfos/' mesechtaName '/' mesechtaName '_' int2str(i) '.png'];
