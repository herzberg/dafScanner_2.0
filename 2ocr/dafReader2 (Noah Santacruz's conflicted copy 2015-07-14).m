...
for k = 19:length(mesachtas) % start at 3 bc of . and ..
    disp('----------------------------------------------');
    ...
    [ startPages(2), endPages(2) ] = getPages( 'rashi', mesachta );
    [ startPages(3), endPages(3) ] = getPages( 'tosfos', mesachta );
    startPage = min(startPages);
    endPage = max(endPages);
    if startPage ~= startPages(1)
        disp('fail') 
    end
    if endPage ~= endPages(1)
        disp('fail') 
    end
